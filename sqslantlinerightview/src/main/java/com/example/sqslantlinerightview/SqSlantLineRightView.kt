package com.example.sqslantlinerightview
...
fun Canvas.drawSqSlantLineRight(scale : Float, w : Float, h : Float, paint : Paint) {
...
fun Canvas.drawSSLRNode(i : Int, scale : Float, paint : Paint) {
    ...
    drawSqSlantLineRight(scale, w, h, paint)
}

class SqSlantLineRightView(ctx : Context) : View(ctx) {
    ...
    data class SqSlantLineRight(var i : Int) {
    ...
    data class Renderer(var view : SqSlantLineRightView) {

        private val animator : Animator = Animator(view)
        private val sslr : SqSlantLineRight = SqSlantLineRight(0)
        private val paint : Paint = Paint(Paint.ANTI_ALIAS_FLAG)

        fun render(canvas : Canvas) {
            canvas.drawColor(backColor)
            sslr.draw(canvas, paint)
            animator.animate {
                sslr.update {
                    animator.stop()
                }
