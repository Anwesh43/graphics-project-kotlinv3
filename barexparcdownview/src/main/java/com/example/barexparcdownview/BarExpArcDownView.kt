package com.example.barexparcdownview
...
fun Canvas.drawBarExpArcDown(scale : Float, w : Float, h : Float, paint : Paint) {
    ...
    drawXY(w / 2, h / 2 + (h / 2 + size / 2) * dsc(3)) {
        drawXY(0f, 0f) {
            rotate(rot * dsc(2))
            drawRect(RectF(-size * dsc(1), 0f, 0f, size * 0.5f), paint)
        }
...
fun Canvas.drawBEADNode(i : Int, scale : Float, paint : Paint) {
    ...
    drawBarExpArcDown(scale, w, h, paint)
}

class BarExpArcDownView(ctx : Context) : View(ctx) {
    ...
    data class BarExpArcDown(var i : Int) {
    ...
    data class Renderer(var view : BarExpArcDownView) {

        private val animator : Animator = Animator(view)
        private val bead : BarExpArcDown = BarExpArcDown(0)
