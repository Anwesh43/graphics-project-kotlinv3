...
class SqRotShiftDownView(ctx : Context) : View(ctx) {
    ...
    data class SRSDNode(var i : Int = 0, val state : State = State()) {

        private var next : SRSDNode? = null
        private var prev : SRSDNode? = null
        ...
        fun draw(canvas : Canvas, paint : Paint) {
            canvas.drawSRSDNode(i, state.scale, paint)
        }
        ...
        fun getNext(dir : Int, cb : () -> Unit) : SRSDNode {
            var curr : SRSDNode? = prev
            if (dir == 1) {
                curr = next
            }
            if (curr != null) {
    ...
    data class SqRotShiftDown(var i : Int) {

        private var curr : SRSDNode = SRSDNode(0)
